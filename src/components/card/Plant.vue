  ...
  >
    <NuxtLink
      v-stat="{
        type: 'click',
        category: 'plant_card',
        label: `${fromPageList}-厂房卡片-${data.id}`,
        action: 'click',
      }"
      :to="`/plant/detail_${data.id}`"
    ...
    >
      ...
      <div class="pt-[.7292vw] pb-[.8333vw] px-[.8333vw]">
        ...
        <!-- tag -->
        <div
          v-if="props.data.featureName?.length"
          class="flex items-center mt-[.625vw] flex-wrap gap-[.4167vw] 1360:h-[1.0417vw] h-[15px] overflow-hidden"
          :title="props.data.featureName.join(',')"
        >
          <div
            v-for="tag in props.data.featureName"
            :key="tag"
        ...
        >
          <div
            v-if="!plantList.includes(props.data.id as string)"
            v-stat="{
              type: 'click',
              category: 'plant_card',
...
import type { IPlantRecord } from '@/models/list'
...
const featureLine = computed(() => {
  const tempList = [
    props.data.parkStreetName,
    `共${props.data.totalNumber}层`,
    translateDict(String(props.data.structureName), plant_structure.value),
    translateDict(String(props.data.newOld), plant_new_old.value),
  ].filter((i) => !!i)
...
const { plantList } = storeToRefs(useCartStore())
const { addPlant, removePlant } = useCartStore()
// 获取字典数据
// console.log(dict.value)
const plant_structure = computed(() => {
  return dict.value?.plant_structure
})

const plant_new_old = computed(() => {
  return dict.value?.plant_new_old
})

// console.log('厂房卡片', dict.value)
...
function addCartAction() {
  addPlant(props.data.id as string)
}
/**
 * @description: 移除预选址清单
 * @return {*}
...
function removeCartAction() {
  removePlant(props.data.id as string)
  emits('delete')
}
/**
 * @description: 下载报告
